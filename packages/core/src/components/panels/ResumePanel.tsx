...
function generateMarkdown(config: any, allSkills: Record<string, Skill[]>, entries: ExperienceEntry[], topProjects: Project[]): string {
  const lines: string[] = [];
  ...
  if (config.languages?.length) {
    lines.push(`**Languages:** ${config.languages.map((l: any) => `${l.language} (${l.level})`).join(' · ')}`);
  }
...
export default function ResumePanel() {
  const { config, experience, skills, projects } = usePortfolioData();
  const entries = experience as ExperienceEntry[];
  const allSkills = skills as unknown as Record<string, Skill[]>;
  const topProjects = useMemo(() => (projects as Project[]).filter(p => topProjectSlugs.includes(p.slug)), [projects]);
  ...
  const handleDownloadMd = useCallback(() => {
    const md = generateMarkdown(config, allSkills, entries, topProjects);
    const safeName = (config as any).name.replace(/\s+/g, '_');
    downloadFile(md, `${safeName}_Resume.md`, 'text/markdown');
  }, [config, allSkills, entries, topProjects]);
